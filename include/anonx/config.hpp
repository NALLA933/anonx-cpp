...
//
// TDLib session architecture:
//   * TDLib does not use Pyrogram base64 string sessions. It uses interactive
//     phone auth and persists SQLite/binlog session keys into a database directory.
//   * SESSION / SESSION_NAME / DATA_DIR specify the assistant session name or directory.
//   * PHONE_NUMBER / PHONE_NUMBER2 / PHONE_NUMBER3 specify assistant phone numbers
//     for one-time TDLib login (or interactive prompt on CLI).
...
public:
    ...
    std::string  session1;         // SESSION / SESSION_NAME (assistant 1 session name or path)
    std::string  session2;         // SESSION2   (optional)
    std::string  session3;         // SESSION3   (optional)

    // TDLib session persistence directory and session name
    std::string  session_name = "assistant";        // SESSION_NAME
    std::string  data_dir     = "./data/tdlib_session"; // DATA_DIR / SESSION_DIR

    // Assistant phone numbers for TDLib login
    ...
    // Number of configured assistant userbots (non-empty SESSION/SESSION_NAME/PHONE_NUMBER values), 1..3.
