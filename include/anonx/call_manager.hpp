...
public:
    ...
    struct Callbacks {
        // Fetch a media file for a video id (delegates to the YouTube service).
        // Returns the local path, or nullopt on failure. Mirrors yt.download.
    ...
    // The three collaborators are borrowed references and must outlive the
    ...
    // On a start, the media file is fetched on demand if not already present.
