...
struct PlayRequest {
    ...
    bool hasReply = false;               // replying to a media message
...
// `api`/`db`; it neither sends messages nor leaves chats — the caller acts on
// `gate`. The assistant join/unban/invite block from checkUB is intentionally
// NOT ported here (it needs assistant-account management wired in a later
// integration step); see plugins.cpp for the note.
...
// no URL token is present. NOTE: entity-based extraction (hyperlinked text, or a
// URL inside a replied-to message) is deferred — MessageContext carries no
// entities yet.
