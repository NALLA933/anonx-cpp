...
PlayPreflight runPlayPreflight(BotApi& api, Database& db, Queue& queue,
    ...
    // 11) DEFERRED: checkUB's "join the assistant / unban / invite" block runs
    //     here when the chat has no active call. It needs assistant-account
    //     management (get_client, join_chat, unban_chat_member,
    //     export_chat_invite_link) that is wired in a later integration step, so
    //     it is intentionally omitted from the Phase 6a preflight.
