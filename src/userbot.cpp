...
bool Userbot::bootAll(int timeoutMs) {
    if (clients_.empty() && !specs_.empty()) {
        for (const auto& spec : specs_) {
            ...
            if (spec.phoneProvider) {
                opts.phoneProvider = spec.phoneProvider;
            } else if (opts.phoneNumber.empty() && interactive_) {
                opts.phoneProvider = stdinPrompt("Enter phone number for " + spec.name + " (e.g. +1234567890): ");
            }
